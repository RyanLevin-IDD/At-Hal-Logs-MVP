...
namespace At_Hal_Logs
{
    public partial class AppWindow : Form
    {
        ...
        private void Login(WebDriverWait wait, ChromeDriver driver)
        {
            ...
            {
                try { var e = drv.FindElement(By.Id("email-input")); return e.Displayed ? e : null; } catch { return null; }
            });
            var passwordInput = wait.Until(drv =>
            {
                try { var e = drv.FindElement(By.Id("password-input")); return e.Displayed ? e : null; } catch { return null; }
            });
            var loginButton = wait.Until(drv =>
            {
                try { var e = drv.FindElement(By.CssSelector("button[type='submit']")); return e.Displayed && e.Enabled ? e : null; } catch { return null; }
            });
        ...
        private void NavigateToLogs(ChromeDriver driver)
        {
            try
            {
                ...
                var acceseLogsTab = WaitForElement(driver, Globals.acceseLogsTab_Xpath);
                acceseLogsTab.Click();
                Thread.Sleep(1000);
            }
            catch (Exception ex)
        ...
        private List<Dictionary<string, string>> ScrapeAccessLogs(ChromeDriver driver)
        {
        ...
        private async Task ProcessDomainLogs(ChromeDriver driver, string domain, WebDriverWait wait)
        {
            ...
            if (websiteDahsboardReady)
            {
                ...
                if (logsFound)
                {
                    ...
                    ApplyFilters(driver);
                    //Calculate pages
                    List<Dictionary<string, string>> allLogs = new List<Dictionary<string, string>>();
                    int totalPages = CalculateLogPages(driver);
                    //Append logs from each page
                    for (int page = 1; page <= totalPages; page++)
                    {
                        wait.Until(d => ((IJavaScriptExecutor)d).ExecuteScript("return document.readyState").Equals("complete"));
                        await Task.Delay(1000);
                        var pageLogs = ScrapeAccessLogs(driver);
                    ...
                    {
                        //Output Log
                        await SendInfoLogsToSheetAPI($"Error - Scrapper found:{rowsInResults} results and Hoistinger is showing:{rowsInHoistinger}, Please debugg code , Domain: {domainName}");
                    }
                    //Send to Sheet and check for errors
                    if (rowsInResults > 0)
                    {
        ...
        private bool WaitForElementByCssClass(IWebDriver drv, string cssClass, string textToSearch)
        {
            ...
            var element = wait.Until(drv =>
            {
                try
                {
                    var e = drv.FindElements(By.CssSelector(cssClass))
                                 .FirstOrDefault(e => e.Text.Contains(textToSearch, StringComparison.OrdinalIgnoreCase));
                    if (e != null)
                    {
                        return true;
                    }
