...
namespace At_Hal_Logs
{
    public partial class AppWindow : Form
    {
        ...
        private async Task ProcessDomainLogs(ChromeDriver driver, string domain, WebDriverWait wait)
        {
            ...
            async Task<List<Dictionary<string, string>>> ScrapeAllPagesAsync()
            {
            ...
            //Scrape first time
            List<Dictionary<string, string>> allLogs = await ScrapeAllPagesAsync();
            ...
            {
                // Refresh and retry
                IWebElement timeFilterButton = WaitForElement(driver,Globals.logsTableTimeFilter_Xpath);
                timeFilterButton.Click();
                WaitForElement(driver, Globals.tableElement_Xpath);
                await Task.Delay(1500);
                allLogs = await ScrapeAllPagesAsync();
                rowsInResults = allLogs.Count;
            }*/
            //Retry until succese
            while (rowsInHoistinger != rowsInResults)
            {
                ...
                // Recount the logs you scraped so far
                rowsInResults = allLogs.Count;
            }
            ...
            if (rowsInHoistinger != rowsInResults)
            {
                await SendInfoLogsToSheetAPI($"Error - Scrapper found:{rowsInResults} results and Hoistinger is showing:{rowsInHoistinger}, Please debug code. Domain: {domainName}");
            }
        ...
        private bool CaptchaDetecter(ChromeDriver driver)
        {
            try
            {
                ...
                {
                    txtLogs.Text += Environment.NewLine + "[CAPTCHA DEBUG] Input NOT in page source";
                }
                ...
                {
                    txtLogs.Text += Environment.NewLine + "[CAPTCHA DEBUG] Container div not found";
                    return false;
                }
            }
            catch (Exception ex)
